...
class Treap {
 public:
  ...
  vector<int> getHash() const {
    auto hash = vector<int>(Node::MOD_COUNT);
    for (int i = 0; i < Node::MOD_COUNT; ++i)
 ...
 private:
  class Node {
   public:
    ...
    static const int MOD_COUNT = 3;
    static const int MOD[MOD_COUNT];
    ...
    void update() {
      ...
      ++size;
      for (int i = 0; i < MOD_COUNT; ++i)
        hash[i] = (1LL * hash[i] * basePow(i, 1) + value) % MOD[i];
      if (right != NULL) {
        size += right->size;
        for (int i = 0; i < MOD_COUNT; ++i)
          hash[i] = (1LL * hash[i] * basePow(i, right->size) + right->hash[i]) % MOD[i];
      }
    }
   ...
   private:
    static int basePow(const int i, const int p) {
      static vector<int> BASE_POW[MOD_COUNT];
      if (BASE_POW[i].empty())
        BASE_POW[i].push_back(1);
      while (int(BASE_POW[i].size()) <= p)
        BASE_POW[i].push_back((1LL * BASE_POW[i].back() * BASE) % MOD[i]);
      return BASE_POW[i][p];
    }
  };
...
const int Treap::Node::MOD[Treap::Node::MOD_COUNT] = {49979693, 941083987, 1000000007};
