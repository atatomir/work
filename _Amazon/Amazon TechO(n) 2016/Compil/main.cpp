...
vint i("i"), j("j"), aux("aux"), aux2("aux2"), x("x"), y("y"),k("k");
...
vint bestdistpa("bestdistpa");
vint xparez("xparez"), yparez("yparez");

varr dist2("dist2", 55 * 55);
vint actdist2("actdist2");
...
void get_dir(vint xs, vint ys, vint xd, vint yd, vint sol) {
    ...
    set(aux_room, aux, 2); // nicu - am pus 1, inainte era -1
...
void get_nearest_de_alex_vraja() {
    ...
    set(aux, n);
    aux *= m;
    copy_it(dist2, dist, aux);

    set(bestpair, i);
    set(bestdist, 1000000);
   ...
   // pr("jobs @ 21 =");
    //IF();
        ...
        IF();
            step == 1;
            IF();
                //set(current_pa,8);            // 8 se foloseste
                set(current_pa,8);
