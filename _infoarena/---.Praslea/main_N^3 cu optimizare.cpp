...
int solve(int t_begin,int t_end){
    ...
    for (int i=1;i<=n;i++){
        ...
        for (int pos = 0;pos<=rmax;pos++) dp[ind][pos] = dp[ind-1][pos];
        for (int pos = rmax-act_risk;pos>=0;pos--){
            if (dp[ind][pos+act_risk]>=dp[ind][pos]+act_force) continue;
            dp[ind][pos+act_risk] = dp[ind][pos]+act_force;
        }
    ...
    int ans = 0;
    for (int i=0;i<=rmax;i++) ans = max(ans,dp[ind-1][i]);
    return ans;
}
