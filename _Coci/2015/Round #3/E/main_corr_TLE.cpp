...
int n, k, m, i, x, op, a, b, def_step;

long long aint_data[maxN << 2], need;
aint work;
...
int getAns() {
    ...
    for (i = 1; i <= n; i++) {
        ...
        for (int step = def_step; step > 0; step >>= 1) {
            if (ans + step <= n)
